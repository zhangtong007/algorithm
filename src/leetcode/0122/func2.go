...
// 套用188通用模板  通用模板虽然可以解，但是k==n时可能超出最大内存限制
func maxProfit2(prices []int) int {
	// 每天都可以选择买，且可以同一天买，最多可以交易n次
	return maxProfitGeneral(len(prices), prices)
}
	...
	for i := 1; i < len(dp); i++ {
		for j := range dp[i] {
			for k := range dp[i][j] {
				// 买
				if k > 0 {
					dp[i][1][k] = max1(dp[i][1][k], dp[i-1][0][k-1]-prices[i])
				}
				// 卖
				dp[i][0][k] = max1(dp[i][0][k], dp[i-1][1][k]+prices[i])
				// 啥也不干
				dp[i][j][k] = max1(dp[i][j][k], dp[i-1][j][k])
			}
		}
	}
